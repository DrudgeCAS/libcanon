 ...
 * orbits of a node's own symmetry group.  Here node 3 has six valences with
 * the symmetry group generated by the single permutation (0 1)(2 3 4 5), whose
 * orbits are {0, 1} and {2, 3, 4, 5}.  Applying an element of that group to
 * node 3's valences gives another spelling of the very same eldag, so the two
 * have to reach the same canonical form.
 *
 * Groups like this one need the orbit relation to be closed rather than swept
 * over once.  The symmetries used in the other tests here are all small enough
 * that a single sweep happens to give the right answer, so this case is needed
 * to keep the distinction covered.
 */

TEST(Test_multi_orbit_node_symm, respects_the_full_orbits)
{
    const size_t n_nodes = 4;
    const size_t n_valences = 6;

    // The generator (0 1)(2 3 4 5), given by its pre-image array.
    Simple_perm gen{ 1, 0, 5, 2, 3, 4 };
    auto node_symm = build_sims_sys<Simple_perm>(n_valences, { gen });
    ASSERT_TRUE(node_symm);

    Node_symms<Simple_perm> symms(n_nodes, nullptr);
    symms[3] = node_symm.get();

    // Three leaves of one colour, and one node of another carrying all the
    // valences.
    std::vector<size_t> colours{ 0, 0, 0, 1 };

    Eldag eldag{};
    for (size_t i = 0; i < n_nodes - 1; ++i) {
        eldag.update_ia();
    }
    eldag.edges = std::vector<size_t>{ 2, 1, 1, 2, 2, 1 };
    eldag.update_ia();

    // The square of the generator, (2 4)(3 5), is in the group as well.  Act
    // with it on node 3's valences only, leaving the nodes themselves alone.
    Simple_perm sq{ 0, 1, 4, 5, 2, 3 };
    Borrowed_node_perms<Simple_perm> local(n_nodes, nullptr);
    local[3] = &sq;
    Eldag permuted
        = act_eldag(Simple_perm(n_nodes), local, eldag);

    // The two really are different spellings of the same eldag.
    EXPECT_NE(eldag, permuted);

    auto res = canon_eldag(eldag, symms, [&](auto i) { return colours[i]; });
    auto res_permuted
        = canon_eldag(permuted, symms, [&](auto i) { return colours[i]; });
