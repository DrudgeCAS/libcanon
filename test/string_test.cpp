...
/** Enumerates every element of the group given by a transversal system.
 */

static std::vector<Simple_perm> enumerate_group(
    const Sims_transv<Simple_perm>& transv, size_t size)
{
    std::vector<Simple_perm> res{ Simple_perm(size) };

    for (const Sims_transv<Simple_perm>* level = &transv; level != nullptr;
         level = level->next()) {
        std::vector<Simple_perm> extended{};
        for (const auto& base : res) {
            extended.push_back(base);
            for (const auto& perm : *level) {
                extended.push_back(base | perm);
            }
        }
        res.swap(extended);
    }
...
/** Tests that the returned automorphism group can canonicalize again.
 *
 * `canon_string` conjugates the automorphism group into the canonical form
 * before returning it, and callers such as `Eldag_coset::refine_nodes` feed
 * that group straight back in on the next refinement round.
 *
 * The refiner prunes greedily on the value at each level`s target, which is a
 * minimization only when every level`s target is the earliest point moved by
 * that level`s subgroup.  Conjugation moves the targets, so the returned chain
 * has to be rebuilt for that property to survive.
 *
 * Here the result of canonicalizing with the returned group is compared
 * against the true minimum over the group, obtained by enumeration.
 */

TEST(Test_returned_aut, can_be_used_for_canonicalization_again)
{
    const size_t size = 6;

    // The group generated by (0 1)(2 3 4 5), whose Sims chain has more than
    // one level, so that conjugation can disturb the order of the targets.
    auto group = build_sims_sys<Simple_perm>(
        size, { Simple_perm{ 1, 0, 5, 2, 3, 4 } });
    ASSERT_TRUE(group);

    // Canonicalize one string just to obtain the conjugated automorphism
    // group that comes back with it.
    std::vector<size_t> seed{ 2, 2, 2, 0, 2, 0 };
    auto first = canon_string(seed, *group);
    ASSERT_TRUE(first.second);
    const auto& aut = *first.second;

    // Now canonicalize a second string with that group, the way the eldag
    // refiner does on its next round.
    std::vector<size_t> input{ 2, 3, 0, 2, 0, 4 };
    auto res = canon_string(input, aut);
    auto got = act_string<std::vector<size_t>>(res.first, input);

    // The true minimum of the string over the very same group.
    auto expected = input;
    for (const auto& perm : enumerate_group(aut, size)) {
        auto image = act_string<std::vector<size_t>>(perm, input);
        if (image < expected) {
            expected = image;
        }
    }
